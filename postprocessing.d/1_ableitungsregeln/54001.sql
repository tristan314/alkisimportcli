...
FROM (
	...
	FROM ax_vegetationsmerkmal o
	LEFT OUTER JOIN ap_ppo p ON o.gml_id = ANY(p.dientzurdarstellungvon) AND p.art='BWS' AND p.endet IS NULL
	LEFT OUTER JOIN ap_darstellung d ON o.gml_id = ANY(d.dientzurdarstellungvon) AND d.art='BWS' AND d.endet IS NULL
...
FROM ax_vegetationsmerkmal o
JOIN ap_ppo p ON o.gml_id = ANY(p.dientzurdarstellungvon) AND p.art='ZUS' AND p.endet IS NULL
...
FROM ax_vegetationsmerkmal o
LEFT OUTER JOIN ap_pto t ON o.gml_id = ANY(t.dientzurdarstellungvon) AND t.art='BWS' AND t.endet IS NULL
LEFT OUTER JOIN ap_darstellung d ON o.gml_id = ANY(d.dientzurdarstellungvon) AND d.art='BWS' AND d.endet IS NULL
