...
FROM (
	...
	FROM ax_einrichtunginoeffentlichenbereichen o
	LEFT OUTER JOIN ap_ppo p ON o.gml_id = ANY(p.dientzurdarstellungvon) AND p.art='ART' AND p.endet IS NULL
...
SELECT
	...
	coalesce(t.advstandardmodell||t.sonstigesmodell,o.advstandardmodell||o.sonstigesmodell) AS modell
FROM ax_einrichtunginoeffentlichenbereichen o
LEFT OUTER JOIN ap_pto t ON o.gml_id = ANY(t.dientzurdarstellungvon) AND t.art='KMA' AND t.endet IS NULL
LEFT OUTER JOIN ap_darstellung d ON o.gml_id = ANY(d.dientzurdarstellungvon) AND d.art='KMA' AND d.endet IS NULL
