...
import() {
	...
	*)
	...
	opt="$opt -skipfailures -ds_transaction --config PG_USE_COPY $USECOPY -nlt CONVERT_TO_LINEAR"
...
progress() {
	...
	if (( elapsed > 0 )); then
		...
		if (( t0 < t1 )); then
			...
			echo "REMAINING: $(memunits $remaining_size) $(( remaining_size * 100 / total_size ))% $(timeunits $remaining_time) ETA:$(date -r "$eta")"
		else
